...
contract RewardPointFactory is ISignerInfo, Ownable {
    ...
    function create(address _owner, uint8 _decimals, string memory _name, string memory _symbol)
        external
        onlyOwner
        returns (address)
    {
    ...
    function _create(address _owner, uint8 _decimal, string memory _name, string memory _symbol)
        private
        returns (address)
    {
        address _proxy = address(
            new RewardPointProxy(
                _beacon, abi.encodeCall(RewardPoint.initialize, (_owner, address(this), _decimal, _name, _symbol))
            )
        );
